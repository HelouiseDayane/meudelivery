  ...
  useEffect(() => {
    const validateSavedBranch = async () => {
      try {
        ...
        const branchList: any[] = Array.isArray(branchesResponse) ? branchesResponse : [];
        const activeBranches = branchList.filter((b: any) => b.is_active !== false);

        // Se só tem uma filial, seleciona automaticamente sem mostrar modal
        if (activeBranches.length === 1) {
          const only = activeBranches[0];
          setSelectedBranch(only);
          localStorage.setItem('selected_branch', JSON.stringify(only));
          setShowBranchModal(false);
          return;
        }

        // Mais de uma filial: verificar se já há seleção salva
        const savedBranch = localStorage.getItem('selected_branch');
        if (savedBranch) {
          const parsed = JSON.parse(savedBranch);
          const branchExists = activeBranches.find((b: any) => b.id === parsed.id);
          if (branchExists) {
        ...
        // Nenhuma filial salva válida: abrir modal
