...
const PublicMenu = () => {
  ...
  // Estado para modal de opções de sabor (2 sabores quando qty >= 2)
  const [flavorModal, setFlavorModal] = useState<{
    open: boolean;
    product: any | null;
    selectedFlavor: 'single' | 'double';
    quantity: number;
  }>({ open: false, product: null, selectedFlavor: 'single', quantity: 1 });

  const handleOpenFlavorModal = (product: any, quantity: number) => {
    setFlavorModal({ open: true, product, selectedFlavor: 'single', quantity });
  };

  const handleConfirmFlavorModal = async () => {
    const { product, selectedFlavor, quantity } = flavorModal;
    if (!product) return;
    const hasTwoFlavor = !!product.two_flavor_price;
    const price = selectedFlavor === 'double' && hasTwoFlavor
      ? product.two_flavor_price
    ...
    const productWithOptions = {
      ...
      isPromotion: false,
      name: selectedFlavor === 'double' ? `${product.name} (2 Sabores)` : product.name,
    };
    setFlavorModal(prev => ({ ...prev, open: false }));
    await handleAddToCart(productWithOptions, quantity);
  };
  ...
  const formatProducts = (Array.isArray(publicProducts) ? publicProducts : []).map((product: any) => {
    ...
    return {
      ...
      two_flavor_price: product.two_flavor_price ? Number(product.two_flavor_price) : null,
      has_chocolate_syrup: Boolean(product.has_chocolate_syrup),
      has_ninho_syrup: Boolean(product.has_ninho_syrup),
    };
  });
  ...
  return (
    <div className="container mx-auto px-4 py-8">
      ...
      {selectedBranch && (
        ...
        {storeConfigState?.logoHorizontal ? (
          <div className="flex justify-center mb-4">
            <img
              src={storeConfigState.logoHorizontal}
              alt={storeConfigState.storeName || 'Logo'}
      ...
      <Alert className="mb-6 border-2 border-orange-500 bg-orange-50">
        ...
        <AlertDescription className="font-medium text-orange-800">
          ⏰ <strong>Lembre-se:</strong> Após adicionar produtos ao carrinho, você tem apenas <strong>5 minutos</strong> para finalizar sua compra.
                  ...
                  <div className="flex flex-col gap-2">
                    ...
                    <div className="flex gap-2">
                      ...
                      <Button
                        ...
                        onClick={() => {
                          if (lojaFechada || isIndisponivel) return;
                          const qty = getLocalQuantity(product.id);
                          const hasTwoFlavor = !!product.two_flavor_price;
                          // Abre modal de 2 sabores apenas quando qty >= 2 e produto tem preço diferenciado
                          if (hasTwoFlavor && qty >= 2) {
                            handleOpenFlavorModal(product, qty);
                          } else {
      ...
      {/* Modal de 2 Sabores - aparece apenas quando qty >= 2 e produto tem preço diferenciado */}
      {flavorModal.open && flavorModal.product && (
        <Dialog open={flavorModal.open} onOpenChange={(open: boolean) => setFlavorModal(prev => ({ ...prev, open }))}>
          <DialogContent className="sm:max-w-md">
            <DialogHeader>
              <DialogTitle>Escolha a quantidade de sabores 🍰</DialogTitle>
            </DialogHeader>
            <div className="space-y-5 py-2">
              <p className="font-medium text-base">{flavorModal.product.name} × {flavorModal.quantity}</p>

              <div>
                <p className="text-sm font-semibold mb-3">Como deseja?</p>
                <div className="flex gap-3">
                  <button
                    type="button"
                    onClick={() => setFlavorModal(prev => ({ ...prev, selectedFlavor: 'single' }))}
                    className={`flex-1 py-3 rounded-lg border-2 font-medium transition-all ${flavorModal.selectedFlavor === 'single' ? 'border-primary bg-primary/10 text-primary' : 'border-gray-200 bg-white text-gray-700'}`}
                  >
                    <div className="text-lg">1 Sabor</div>
                    <div className="text-sm text-gray-500">
                      R$ {(flavorModal.product.promotionPrice || flavorModal.product.price)?.toFixed(2).replace('.', ',')} cada
                    </div>
                  </button>
                  <button
                    type="button"
                    onClick={() => setFlavorModal(prev => ({ ...prev, selectedFlavor: 'double' }))}
                    className={`flex-1 py-3 rounded-lg border-2 font-medium transition-all ${flavorModal.selectedFlavor === 'double' ? 'border-primary bg-primary/10 text-primary' : 'border-gray-200 bg-white text-gray-700'}`}
                  >
                    <div className="text-lg">2 Sabores</div>
                    <div className="text-sm text-orange-600 font-bold">
                      R$ {Number(flavorModal.product.two_flavor_price).toFixed(2).replace('.', ',')} cada
                    </div>
                  </button>
                </div>
                <p className="text-xs text-muted-foreground mt-2">
                  💡 A calda poderá ser escolhida no checkout.
                </p>
              </div>

              <Button
                className="w-full bg-primary hover:bg-primary/90 text-white"
                onClick={handleConfirmFlavorModal}
              >
