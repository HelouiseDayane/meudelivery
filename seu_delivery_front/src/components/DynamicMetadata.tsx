...
export const DynamicMetadata = () => {
  ...
  useEffect(() => {
    ...
    // Atualiza o favicon dinamicamente se disponível (forçar https)
    const favicon = document.querySelector('link[rel="icon"]') as HTMLLinkElement;
    if (favicon && storeConfig.logoIcon && storeConfig.logoIcon.startsWith('http')) {
      favicon.href = storeConfig.logoIcon.replace(/^http:/, 'https:');
    }
    ...
    const manifestData = {
      ...
      icons: [
        {
          src: storeConfig.logoIcon.startsWith('http') ? storeConfig.logoIcon.replace(/^http:/, 'https:') : "/icone-selobrunocakes.ico",
          sizes: "192x192",
