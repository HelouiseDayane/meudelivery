...
export function ProductsManagement() {
  ...
  const handleEdit = (product: any) => {
    ...
    setFormData({
      ...
      twoFlavorPrice: product.two_flavor_price !== undefined && product.two_flavor_price !== null ? product.two_flavor_price.toString().replace('.', ',') : '',
      hasChocolateSyrup: product.has_chocolate_syrup || false,
      hasNinhoSyrup: product.has_ninho_syrup || false,
    });
...
const handleSubmit = async (e: React.FormEvent) => {
  ...
  const productData: any = {
    ...
    branch_id: isMaster ? parseInt(Object.keys(branchStocks)[0]) : userBranchId, // Primeira filial ou filial do admin
  };
  ...
  // two_flavor_price: envia o valor numérico se preenchido, ou string vazia para limpar
  const rawTwoFlavor = formData.twoFlavorPrice ? formData.twoFlavorPrice.replace(/[^\d,]/g, '').replace(',', '.') : '';
  const twoFlavorValue = rawTwoFlavor ? parseFloat(rawTwoFlavor) : null;
  productData.two_flavor_price = (!twoFlavorValue || isNaN(twoFlavorValue)) ? '' : twoFlavorValue;
  productData.has_chocolate_syrup = formData.hasChocolateSyrup;
  productData.has_ninho_syrup = formData.hasNinhoSyrup;

  try {
    if (editingProduct) {
      // Atualiza produto existente
      const updatedProduct = await adminApi.updateProduct(editingProduct.id, productData);
      // Atualiza estado local imediatamente (forma funcional evita closure stale)
      setAdminProducts(prev => prev.map(p =>
        p.id === editingProduct.id
          ? { ...p, ...updatedProduct, is_active: updatedProduct.is_active ?? productData.is_active }
          : p
      ));
      ...
      <div className="flex justify-between items-center">
        ...
        <div className="flex gap-2">
            {(isMaster || isAdmin) && (
              ...
              {/* Dois Sabores */}
              <div>
                <Label htmlFor="twoFlavorPrice">Preço para 2 Sabores (opcional)</Label>
                <Input
                  id="twoFlavorPrice"
                  type="text"
                  placeholder="Ex: R$ 12,00"
                  value={formData.twoFlavorPrice}
                  onChange={(e) => {
                    const value = formatCurrency(e.target.value);
                    setFormData(prev => ({ ...prev, twoFlavorPrice: value }));
                  }}
                />
                <p className="text-xs text-muted-foreground mt-1">
                  Se preenchido, o cliente verá a opção de escolher 2 sabores por este valor ao adicionar 2 unidades.
                </p>
              </div>
