...
export const StoreSettings: React.FC<StoreSettingsProps> = ({ onBack }) => {
  ...
  const handleSave = async () => {
    try {
      ...
      if (settings.mercado_pago_key) formData.append('mercado_pago_key', settings.mercado_pago_key);
      if (settings.phone) formData.append('phone', settings.phone);
      if (settings.whatsapp) formData.append('whatsapp', settings.whatsapp);
      if (logoHorizontalFile) formData.append('logo_horizontal', logoHorizontalFile);
      if (logoIconFile) formData.append('logo_icon', logoIconFile);

      const token = localStorage.getItem('admin_token') || '';
      const apiBase = (import.meta as any).env?.VITE_API_BASE_URL || '/api';
      const result = await fetch(`${apiBase}/admin/settings`, {
        method: 'POST',
        headers: {
