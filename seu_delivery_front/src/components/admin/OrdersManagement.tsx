...
export const OrdersManagement = () => {
  ...
  const handleMarkAsOutForDelivery = async (orderId: string) => {
    try {
      await adminApi.markAsOutForDelivery(orderId);
      const updatedOrder = { status: 'out_for_delivery' } as Partial<Order>;
  ...
  const selectableIds = filteredOrders
    .filter(o => o.status === 'awaiting_seller_confirmation' || o.status === 'confirmed' || o.status === 'completed' || o.status === 'out_for_delivery' || o.status === 'delivered')
    .map(o => o.id);
                    ...
                    <TableHead>
                      <Checkbox
                        ...
                        style={{
                          ...
                          borderRadius: '4px',
                          backgroundColor: isAllSelected ? '#f97316' : '#ffffff',
                          borderColor: isAllSelected ? '#f97316' : '#6b7280',
                          display: 'block',
                        }}
                      />
                    </TableHead>
                        ...
                        {(order.status === 'confirmed' || order.status === 'completed' || order.status === 'delivered') && (
                          <Checkbox
                            ...
                            style={{
                              ...
                              border: '2.5px solid #6b7280',
                              borderRadius: '4px',
                              backgroundColor: selectedIds.includes(order.id) ? '#f97316' : '#ffffff',
                              borderColor: selectedIds.includes(order.id) ? '#f97316' : '#6b7280',
                              display: 'block',
                            }}
                          />
                        )}
                            ...
                            <Button
                              ...
                              onClick={async () => {
                                try {
                                  ...
                                  updateOrder(order.id, { status: 'confirmed' } as OrderUpdate);
                                  toast.success(`✅ Pagamento do pedido #${order.id} confirmado!`);
                                } catch (err) {
                                  toast.error('Erro ao confirmar pagamento');
                                }
                              }}
                              className="gap-1 !bg-amber-500 hover:!bg-amber-600 !text-white font-semibold"
                            >
