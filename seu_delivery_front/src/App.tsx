...
function AppProvider({ children }: { children: ReactNode }) {
    ...
    const checkCartExpiration = async () => {
      if (cart.length > 0) {
        try {
          ...
          // Só limpa o carrinho se o backend retornar 410 (carrinho expirado no Redis)
          ...
          if (status === 410) {
            ...
            if (typeof window !== 'undefined') {
              window.dispatchEvent(new CustomEvent('cart-expired', {
                detail: { message: 'Seu carrinho expirou! Você tem apenas 5 minutos para escolher seus produtos.' }
              }));
            }
          }
          // Qualquer outro erro (rede, 500, etc.) é ignorado silenciosamente
        }
      }
    };
  ...
  useEffect(() => {
    ...
    // SSE: novos pedidos em tempo real (sem polling)
    const sse = new EventSource('/api/stream/updates');

    sse.addEventListener('order_created', (e: MessageEvent) => {
      try {
        const data = JSON.parse(e.data);
        // Busca o pedido completo no backend e adiciona ao estado
        adminApi.getOrders().then((backendOrders: any[]) => {
          const found = backendOrders.find((o: any) => String(o.id) === String(data.order_id));
          if (found) {
            const transformed = {
              id: String(found.id),
              clientName: found.customer_name || 'Cliente não informado',
              whatsapp: found.customer_phone || '',
              ...
              address: found.address_street || '',
              neighborhood: found.address_neighborhood || '',
              observations: found.observations || '',
              scheduledDate: found.scheduled_date,
              createdAt: found.created_at,
              pixCode: found.payment?.pix_payload ? JSON.parse(found.payment.pix_payload || '{}').copy_paste : null,
              pixExpiresAt: found.checkout_expires_at,
            };
            addOrder(transformed);
            toast.success(`🛒 Novo pedido #${found.id} de ${found.customer_name || 'cliente'}!`);
          }
        }).catch(() => {});
      } catch (_) {}
    });
