...
class CheckoutController extends Controller
{
    ...
    public function removeFromCart(Request $request)
    {
        ...
        if ($reservedQuantity > 0) {
            ...
            Redis::connection('stock')->decrby("product_reserved_{$productId}", $reservedQuantity);
            
            // Garantir que não fique negativo
            $currentReserved = Redis::connection('stock')->get("product_reserved_{$productId}") ?? 0;
            if ($currentReserved < 0) {
                Redis::connection('stock')->set("product_reserved_{$productId}", 0);
            }
        }
        ...
        Redis::connection('stock')->set("product_stock_{$productId}", Product::find($productId)->quantity);
        return response()->json([
    ...
    public function updateCart(Request $request)
    {
        ...
        $reserveKey = "reserve:{$sessionId}:{$productId}";
        $oldQuantity = Redis::connection('stock')->get($reserveKey) ?? 0;
        ...
        if ($quantityDiff > 0) {
            $product = Product::findOrFail($productId);
            $currentStock = Redis::connection('stock')->get("product_stock_{$productId}") ?? $product->quantity;
            $reservedStock = Redis::connection('stock')->get("product_reserved_{$productId}") ?? 0;
        ...
        // Atualizar reserva
        Redis::connection('stock')->incrby("product_reserved_{$productId}", $quantityDiff);
        Redis::connection('stock')->setex($reserveKey, 300, $newQuantity);
    ...
    public function getAllProductsStock(Request $request)
    {
        try {
            ...
            if ($branchId) {
                // Filtra produtos desta filial (por branch_id direto no produto)
                // OU produtos que têm estoque nessa filial na tabela product_stocks
            ...
            foreach ($products as $product) {
                try {
                    // Se filtrou por filial, verifica estoque na tabela product_stocks primeiro
                    if ($branchId) {
                        $branchStock = $product->stocks->where('branch_id', $branchId)->first();
                        if ($branchStock) {
                            $redisStock = $branchStock->quantity;
                        } else {
                            // Fallback: usa quantity do próprio produto
                    ...
                    $productsWithStock[] = [
                        ...
                        'two_flavor_price' => $product->two_flavor_price,
                        'has_chocolate_syrup' => (bool)$product->has_chocolate_syrup,
                        'has_ninho_syrup' => (bool)$product->has_ninho_syrup,
    ...
    public function storeWithPix(Request $request)
    {
        ...
        // Adicionar taxa de entrega (motoboy) - busca o menor preço de motociclista ativo
        $activeMoto = \App\Models\Motorcyclist::where('is_active', true)
            ->where('price_per_delivery', '>', 0)
            ->orderBy('price_per_delivery', 'asc')
            ->first();
        $deliveryFee = $activeMoto ? (float) $activeMoto->price_per_delivery : 0.00;
    ...
    /**
     * ✅ PIX SIMPLES — cria pedido e retorna QR Code com chave PIX fixa
     * POST /api/checkout/pix-simples
     */
    public function checkoutPixSimples(Request $request)
    {
        $data = $request->validate([
            'session_id'           => 'required|string',
            'branch_id'            => 'required|integer|exists:branches,id',
        ...
        // Calcular total com preço atual do banco
        $totalAmount = 0;
        $orderItems  = [];
        foreach ($data['items'] as $item) {
            $productId = (int) $item['product_id'];
            ...
            $product   = Product::find($productId);
            if ($product) {
                $unitPrice    = $product->is_promo && $product->promotion_price !== null
                    ? $product->promotion_price
                    : $product->price;
                $totalPrice   = $unitPrice * $quantity;
                $orderItems[] = compact('productId', 'product', 'unitPrice', 'quantity', 'totalPrice');
                $totalAmount += $totalPrice;
            }
        }

        // Taxa de entrega (menor motoboy ativo)
        $activeMoto  = \App\Models\Motorcyclist::where('is_active', true)
            ->where('price_per_delivery', '>', 0)
            ->orderBy('price_per_delivery', 'asc')
            ->first();
        ...
        try {
            ...
            // Notificar admins via Redis (SSE order_created)
            $orderEvent = [
                'type'          => 'order_created',
                'order_id'      => $order->id,
            ...
            Redis::connection('stock')->publish('order-events', json_encode($orderEvent));

            // Chave PIX fixa
            $pixKey      = 'b805f442-c308-42d6-83fe-4b19db4f048c';
            $beneficiary = 'Gabriel P Costa'; // max 25 chars
            $city        = 'Ilheus';          // max 15 chars, sem acento
            $txid        = 'PEDIDO' . str_pad($order->id, 6, '0', STR_PAD_LEFT); // max 25 chars

            // Gera payload PIX EMV/BR Code (Pix Copia e Cola)
            $pixCopyPaste = $this->generatePixPayload($pixKey, $beneficiary, $city, $txid, $totalAmount);
    ...
    /**
     * Gera payload PIX EMV/BR Code (Pix Copia e Cola) conforme BACEN.
     */
    private function generatePixPayload(string $key, string $name, string $city, string $txid, float $amount): string
    {
        $fmt = fn(string $id, string $val): string =>
            $id . str_pad(strlen($val), 2, '0', STR_PAD_LEFT) . $val;

        // Merchant Account Information (ID 26)
        $mai = $fmt('00', 'BR.GOV.BCB.PIX') . $fmt('01', $key);
        $maiField = $fmt('26', $mai);
        ...
        $payload =
            ...
            $maiField .                       // Merchant Account Info
            ...
            $fmt('62', $fmt('05', $txid)) .  // Additional Data (TxID)
