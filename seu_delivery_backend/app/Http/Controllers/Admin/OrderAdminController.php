...
class OrderAdminController extends Controller {
     ...
     public function markAsDelivered($id)
    {
        ...
        if (!in_array($order->status, ['completed', 'out_for_delivery'])) {
            return response()->json([
                'message' => 'Só é possível marcar como entregue pedidos com status completed ou out_for_delivery.',
    ...
    public function markAsOutForDelivery($id)
    {
        $order = Order::findOrFail($id);
        if ($order->status !== 'completed') {
            return response()->json([
                'message' => 'Só é possível marcar como saiu para entrega pedidos com status completed.',
                'current_status' => $order->status
            ], 400);
        }
     ...
     public function markAsCompleted(Request $request)
    {
        ...
        foreach ($orders as $order) {
            ...
            // Envia WhatsApp via Evolution API da filial
            if ($order->customer_phone) {
                try {
                    $branch       = Branch::find($order->branch_id);
                    $instanceName = $branch?->whatsapp_instance_name ?? null;

                    if ($instanceName && $branch?->whatsapp_status === 'connected') {
                        // Limpa número para formato internacional
                        ...
                        $result = $evolutionApi->sendTextMessage($instanceName, $cleanNumber, $msg);
                        ...
                        Log::warning('WhatsApp não enviado: instância não conectada', [
                            'order_id'  => $order->id,
    ...
    /**
     * Confirmar pagamento manualmente (pending_payment -> confirmed)
