...
class ProductAdminController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        $data = $request->merge($input)->validate([
            ...
            'two_flavor_price'     => 'nullable|numeric',
            'has_chocolate_syrup'  => 'nullable|boolean',
            'has_ninho_syrup'      => 'nullable|boolean',
        ]);
    ...
    public function update(Request $request, $id)
    {
        ...
        // Converter campos booleanos vindos como string
        foreach (["is_promo", "is_new", "is_active", "has_chocolate_syrup", "has_ninho_syrup"] as $boolField) {
            if (isset($input[$boolField])) {
                $input[$boolField] = filter_var($input[$boolField], FILTER_VALIDATE_BOOLEAN);
            }
        }
    ...
    public function toggleActive($id)
{
    ...
    private function syncProductStock(Product $product)
    {
        try {
            ...
            // Sincronizar para todas as filiais usando a chave correta: product_stock_{branchId}_{productId}
            $branches = \App\Models\Branch::all();
            foreach ($branches as $branch) {
                $productStock = \App\Models\ProductStock::where('product_id', $product->id)
                    ->where('branch_id', $branch->id)
                    ->first();
                $qty = $productStock ? $productStock->quantity : $product->quantity;
                $key = "product_stock_{$branch->id}_{$product->id}";
                Redis::connection('stock')->set($key, $qty);
            }

            \Log::info("Produto {$product->id} sincronizado com Redis (todas as filiais)", [
    ...
    public function syncStock()
    {
        ...
        foreach ($branches as $branch) {
            foreach ($products as $product) {
                try {
                    // Busca estoque específico por filial se existir
                    $productStock = \App\Models\ProductStock::where('product_id', $product->id)
                        ->where('branch_id', $branch->id)
                        ->first();
                    $qty = $productStock ? $productStock->quantity : $product->quantity;
                    ...
                    $key = "product_stock_{$branch->id}_{$product->id}";
                    $oldValue = Redis::connection('stock')->get($key);
                    Redis::connection('stock')->set($key, $qty);
                    // Zerar reservado apenas se não houver reservas ativas
                    $reservedKey = "product_reserved_{$branch->id}_{$product->id}";
                    if (Redis::connection('stock')->get($reservedKey) === null) {
                        Redis::connection('stock')->set($reservedKey, 0);
                    }
                    $newValue = Redis::connection('stock')->get($key);

                    if ($newValue !== null && (string)$newValue === (string)$qty) {
                        $synced++;
                        $syncDetails["{$branch->id}_{$product->id}"] = [
                            'success' => true,
                            'name' => $product->name,
                            'branch_id' => $branch->id,
                            'old_value' => $oldValue,
                    ...
                    \Log::error("Erro ao sincronizar produto {$product->id} branch {$branch->id}: " . $e->getMessage());
                }
            }
        }
        ...
        return response()->json([
            ...
            'errors' => $errors,
            'total_combinations' => $products->count() * $branches->count(),
            'details' => $syncDetails
        ]);
    }
}