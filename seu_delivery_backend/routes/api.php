...
Route::post('/checkout/pix-simples', [CheckoutController::class, 'checkoutPixSimples']);
...
// Taxa de entrega pública: retorna o menor price_per_delivery de motociclista ativo
Route::get('/motorcyclists/active-delivery-fee', function () {
    $moto = \App\Models\Motorcyclist::where('is_active', true)
        ->where('price_per_delivery', '>', 0)
        ->orderBy('price_per_delivery', 'asc')
        ->first();
    return response()->json([
        'available' => $moto !== null,
        'fee' => $moto ? (float) $moto->price_per_delivery : null,
    ]);
});
...
Route::prefix('admin')->group(function () {
    ...
    Route::middleware(['auth:sanctum'])->group(function () {
        ...
        Route::patch('/{id}/mark-out-for-delivery', [OrderAdminController::class, 'markAsOutForDelivery']);
        Route::patch('/{id}/confirm-payment', [OrderAdminController::class, 'confirmPayment']);
        ...
        Route::prefix('system')->group(function () {
            ...
            Route::prefix('whatsapp')->group(function () {
                ...
                Route::get('/{branchId}/status', [WhatsAppController::class, 'status']);
                Route::get('/{branchId}/qrcode', [WhatsAppController::class, 'getQrCode']);
                Route::post('/{branchId}/disconnect', [WhatsAppController::class, 'disconnect']);
                Route::post('/{branchId}/refresh-qrcode', [WhatsAppController::class, 'refreshQrCode']);
            });
            ...
            // ── Motociclistas (Entregas) ──────────────────────────────────
            Route::prefix('motorcyclists')->group(function () {
                Route::get('/',                                    [\App\Http\Controllers\Admin\MotorcyclistController::class, 'index']);
                Route::post('/',                                   [\App\Http\Controllers\Admin\MotorcyclistController::class, 'store']);
                Route::get('/{id}',                                [\App\Http\Controllers\Admin\MotorcyclistController::class, 'show']);
                Route::put('/{id}',                                [\App\Http\Controllers\Admin\MotorcyclistController::class, 'update']);
                Route::delete('/{id}',                             [\App\Http\Controllers\Admin\MotorcyclistController::class, 'destroy']);
                Route::patch('/{id}/toggle-active',                [\App\Http\Controllers\Admin\MotorcyclistController::class, 'toggleActive']);
                // Registros de entregas
                Route::get('/{id}/records',                        [\App\Http\Controllers\Admin\MotorcyclistController::class, 'records']);
                Route::post('/{id}/records',                       [\App\Http\Controllers\Admin\MotorcyclistController::class, 'storeRecord']);
                Route::put('/{id}/records/{recordId}',             [\App\Http\Controllers\Admin\MotorcyclistController::class, 'updateRecord']);
                Route::delete('/{id}/records/{recordId}',          [\App\Http\Controllers\Admin\MotorcyclistController::class, 'destroyRecord']);
                Route::get('/{id}/summary',                        [\App\Http\Controllers\Admin\MotorcyclistController::class, 'summary']);
            });
    });
