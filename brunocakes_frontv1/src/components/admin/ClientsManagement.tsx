...
export function ClientsManagement() {
  ...
  const clients = orders.reduce((acc, order) => {
    const existingClient = acc.find(client => client.id === order.clientId);
    if (existingClient) {
      existingClient.totalOrders += 1;
      existingClient.totalSpent += order.total;
      existingClient.lastOrderDate = new Date(order.createdAt) > new Date(existingClient.lastOrderDate) 
        ? order.createdAt 
          ...
          <CardContent className="space-y-4">
            <div className="flex justify-between items-center">
              ...
              <span className="font-semibold">
                {clients.sort((a, b) => b.totalOrders - a.totalOrders)[0]?.name || 'N/A'}
              </span>
            </div>
            <div className="flex justify-between items-center">
              <span className="text-sm text-gray-600">Cliente que mais gastou:</span>
              <span className="font-semibold">
                {clients.sort((a, b) => b.totalSpent - a.totalSpent)[0]?.name || 'N/A'}
              </span>
            </div>
            <div className="flex justify-between items-center">
              <span className="text-sm text-gray-600">Pedidos por cliente (média):</span>
              <span className="font-semibold">
                {totalClients > 0 ? (orders.length / totalClients).toFixed(1) : '0'}
              </span>
            </div>
            <div className="flex justify-between items-center">
